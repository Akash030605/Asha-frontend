...
import { CalendarIcon, X, Plus, Search, Edit2, Trash2 } from "lucide-react";

type Order = {
  _id: string;
  bill_id?: string; // Will be generated by backend or use _id as fallback
  status: string;
  final_amount: number;
  total_amount: number;
  discount: number;
...
const Orders = () => {
  ...
  const loadOrders = async (search = "") => {
    ...
    try {
      ...
      // Changed from /admin/owner/orders to /api/orders
      ...
      // Populate customer and work details for each order
      const populatedOrders = await Promise.all(ordersList.map(async (order: Order) => {
        const populatedOrder = { ...order };
        
        // Populate customer if only customer_id exists
        if (order.customer_id && !order.customer) {
          const customer = customers.find(c => c._id === order.customer_id);
          if (customer) {
            populatedOrder.customer = customer;
          }
        }
        
        // Populate work details if only works array exists
        if (order.works && order.works.length > 0 && (!order.work_details || order.work_details.length === 0)) {
          const workDetails = order.works
            .map(workId => works.find(w => w._id === workId))
            .filter((w): w is Work => w !== undefined);
          populatedOrder.work_details = workDetails;
        }
  ...
  const loadCustomers = async () => {
    try {
      const token = localStorage.getItem("admin_token") || undefined;
      // Changed from /admin/owner/users to /api/customers (adjust based on your backend)
  ...
  const loadWorks = async () => {
    try {
      const token = localStorage.getItem("admin_token") || undefined;
      // Changed from /admin/owner/works to /api/works
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6 border rounded-lg p-4 bg-card">
        <h3 className="font-semibold mb-4">Add New Order</h3>
          ...
          <tbody>
            {loading && orders.length === 0 ? (
              ...
              <tr key={order._id} className="border-t hover:bg-muted/25">
                ...
                <td className="p-3">
                  {order.work_details && order.work_details.length > 0 ? (
                    order.work_details.map(work => (
                      <div key={work._id} className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded mb-1 inline-block mr-1">
                        {work.work_type} (₹{work.fixed_amount.toLocaleString('en-IN')})
                      </div>
                    ))
                  ) : (
