...
export default function ProcessSection() {
  ...
  const cards = steps.map((s) => {
    ...
    return (
      <div
        key={s.num}
        className="proc-card group relative flex-shrink-0 w-[85vw] sm:w-[72vw] md:w-[clamp(300px,34vw,420px)]"
        style={{ opacity: window.matchMedia("(max-width: 767px)").matches ? 1 : 0 }}
      >
        <div
        ...
        >
          ...
          <div className="relative z-10 p-6 md:p-8 lg:p-10 flex flex-col flex-1">
            ...
            <div
              className="w-12 h-12 flex items-center justify-center mb-6 transition-transform duration-500 group-hover:scale-105 group-hover:rotate-[-3deg]"
              style={{ background: `hsl(${s.accentLight})`, border: `1px solid hsl(${s.accent} / 0.12)` }}
            >
              <Icon className="w-5 h-5" style={{ color: `hsl(${s.accent})` }} />
            </div>

            <div className="flex items-center gap-2.5 mb-3">
              <span className="font-mono text-[10px] font-semibold" style={{ color: `hsl(${s.accent})` }}>{s.num}</span>
              <span className="h-px w-4" style={{ background: `hsl(${s.accent} / 0.3)` }} />
              <span className="text-[9px] tracking-[0.22em] uppercase font-medium" style={{ color: `hsl(${s.accent} / 0.7)` }}>
                {s.subtitle}
              </span>
            </div>

            <h3
              className="font-serif font-bold mb-3 tracking-tight transition-colors duration-300 group-hover:text-[hsl(310_68%_38%)]"
              style={{ fontSize: "clamp(1.2rem, 2.2vw, 1.7rem)", color: "hsl(248 28% 9%)", lineHeight: 1.15 }}
            >
              {s.title}
            </h3>
          ...
          <span
            ...
            style={{ fontSize: "clamp(5rem, 12vw, 10rem)", color: "hsl(248 10% 93%)", letterSpacing: "-0.04em", opacity: 0.4 }}
          >
            {s.num}
          </span>
        </div>
    ...
    <section ref={sectionRef} className="overflow-hidden" style={{ background: "hsl(30 14% 97%)" }}>
      <div className="container-pad pt-16 md:pt-20 pb-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 md:mb-10">
          <div>
            <div className="proc-hdr flex items-center gap-3 mb-4 md:mb-5">
              ...
              <span className="text-[10px] tracking-[0.32em] uppercase font-sans" style={{ color: "hsl(248 46% 40%)" }}>The Process</span>
            </div>
            <h2
              className="proc-hdr font-serif font-bold tracking-tight"
              style={{ fontSize: "clamp(1.8rem, 4.5vw, 3.8rem)", lineHeight: 1.02, color: "hsl(248 28% 9%)" }}
            >
              How It{" "}
        ...
        <div className="proc-hdr h-px w-full relative" style={{ background: "hsl(310 10% 89%)" }}>
          <div
            ref={progressRef}
            className="absolute inset-0 origin-left hidden md:block"
            style={{ background: "linear-gradient(90deg, hsl(310 68% 38%), hsl(248 46% 40%))", transform: "scaleX(0)" }}
          />
        </div>
      </div>

      {/* Mobile: horizontal swipe */}
      <div className="md:hidden overflow-x-auto pb-8 pt-2 px-5 snap-x snap-mandatory scrollbar-none flex gap-4">
        {steps.map((s) => {
          const Icon = s.icon;
          return (
            <div
              key={s.num}
              className="snap-start flex-shrink-0 w-[80vw] max-w-[320px] flex flex-col overflow-hidden"
              style={{ background: "hsl(30 18% 99%)", border: "1px solid hsl(310 10% 87%)" }}
            >
              <div className="h-[3px] w-full" style={{ background: `hsl(${s.accent})` }} />
              <div className="p-6 flex flex-col flex-1">
                ...
                <div className="flex items-center gap-2 mb-2">
                  <span className="font-mono text-[10px] font-semibold" style={{ color: `hsl(${s.accent})` }}>{s.num}</span>
                  <span className="h-px w-3" style={{ background: `hsl(${s.accent} / 0.3)` }} />
                  <span className="text-[9px] tracking-[0.2em] uppercase" style={{ color: `hsl(${s.accent} / 0.7)` }}>{s.subtitle}</span>
                </div>
                <h3 className="font-serif font-bold text-lg mb-2" style={{ color: "hsl(248 28% 9%)" }}>{s.title}</h3>
                <p className="text-[13px] leading-relaxed" style={{ color: "hsl(248 12% 46%)" }}>{s.text}</p>
              </div>
            </div>
          );
        })}
      ...
      <div ref={wrapRef} className="hidden md:block overflow-hidden">
        <div
          ref={trackRef}
